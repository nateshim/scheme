...
def do_if_form(expressions, env):
    ...
    # BEGIN Question 13
    true_value = expressions.second.first
    first = expressions.first
    if scheme_false(first) and expressions.second.second == nil:
        return okay
    elif scheme_false(scheme_eval(first, env)):
        return scheme_eval(expressions.second.second.first, env)
    else:
        return scheme_eval(true_value, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        if scheme_numberp(bindings.first.first):
            raise SchemeError
        Pair1 = Pair(bindings.first.first, Pair1) 
        Pair2 = Pair(scheme_eval(bindings.first.second.first, env), Pair2)
        bindings = bindings.second  

    return env.make_child_frame(Pair1, Pair2)
...
def check_formals(formals):
    ...
    while formals != nil:
        if not scheme_symbolp(formals.first):
            raise SchemeError
        if formals.first in new_formals:
            raise SchemeError
        else: 
